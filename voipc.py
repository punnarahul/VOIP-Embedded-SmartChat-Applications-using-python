...
#Tkinter object cretion and displaying background and the title
top = Tk()
...
def emojichanger(labelcatcher,no):
    ...
    img = img.resize((width,height), Image.ANTIALIAS)
    picuturevar =  ImageTk.PhotoImage(img)
    labelcatcher.configure(image=picuturevar)
    labelcatcher.image =picuturevar
...
#Attaching above function which is invoked when close button on tkinter is pressed
...
def call_request():
    try:
        ...
        try:
            sender=AudioSender(socket.gethostbyname(socket.gethostname()),9999)
            recieve=AudioReceiver(socket.gethostbyname(socket.gethostname()),8002)
        ...
        #After creating the threads, setting buttons appropriately
...
class tr(threading.Thread):
    ...
    def run(self):     
        try:
            global idv,gotmsgfromrecv,recentrecievedmessage,otheridv,otheridvstack,myidv,myidvstack,isconnectionalive,Displayingemoji
            while isconnectionalive:
                ...
                if(r=="***Call-Willingness***"):
                    try:
                        print("Request Recieved connecting you....")
                ...
                #Storing Most recent message Recieved
            ...
            if(not closedbyme):
                msg=messagebox.showinfo('Note','Reciever(Server) has left the Chat App is going to be closed..')
                try:
                    destroyer()
                except:
                    pass
...
def button(x="na"):  
    ...
    #if smartframe's is visible then it should not be displayed
...
#For styling the smart buttons from here
style=ttk.Style()
style.theme_use('alt')
